...
/** IU that identifies the Evolve version (Q1 resolved: the host bundle symbolic name). */
...
val genFeatureP2Inf = tasks.register("genFeatureP2Inf") {
    ...
    doLast {
        if (!evolveHostJar.exists())
            throw GradleException("swt-evolve host jar not found: $evolveHostJar " +
                "(build swt-evolve's host jar or pass -PevolveHostJar)")
        val ver = bundleVersion(evolveHostJar)
        val f = outFile.get().asFile
        f.parentFile.mkdirs()
        f.writeText(
            "# Generated by genFeatureP2Inf from ${evolveHostJar.name} — do not edit.\n" +
