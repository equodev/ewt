...
void main() {
  ...
  group('Dart to Java Transpiler', () {
    // Snapshot tests: the .java golden is regenerated by
    // `dart run tool/regenerate_fixtures.dart` whenever the transpiler's
    // intentional output changes. Uses skipFormat so no dependency on the
    // external palantir java-formatter jar.
    Future<void> snapshot(String dartInput, String? classArg, String golden,
        {String? overridePackage}) async {
      final actual = await transpile(dartInput, classArg,
          skipFormat: true, overridePackage: overridePackage);
      final expected = File(golden).readAsStringSync();
      expect(actual, equals(expected));
    }

    test('Counter App Test', () async {
      await snapshot('test/fixtures/counter.dart', 'build/Counter',
          'test/fixtures/counter.java', overridePackage: '');
    });

    test('Simple Widget Test', () async {
      await snapshot('test/fixtures/hello.dart', 'build/Hello',
          'test/fixtures/hello.java', overridePackage: '');
    });

    test('Model Test', () async {
      await snapshot('test/fixtures/model.dart',
          '../examples/src/main/java/dev/equo/Model.java',
          'test/fixtures/model.java');
    });

    test('Popup Test', () async {
      await snapshot('test/fixtures/action_popup_v9.dart',
          '../examples/src/main/java/dev/equo/KnowledgeFloating.java',
          'test/fixtures/action_popup_v9.java');
    });

    test('View Test', () async {
      await snapshot('test/fixtures/knowledge_view_v57.dart',
          '../examples/src/main/java/dev/equo/KnowledgeView.java',
          'test/fixtures/knowledge_view_v57.java');
    });

    test('Hacka Test', () async {
      await snapshot('test/fixtures/hacka.dart',
          '../examples/src/main/java/dev/equo/Hacka.java',
          'test/fixtures/Hacka.java');
    });

    test('HackaSS Test', () async {
      await snapshot('test/fixtures/temp_dart.dart',
