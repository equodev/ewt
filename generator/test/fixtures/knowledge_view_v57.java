...
.fontSize(12)
.color(Colors.grey()))), SizedBox()
.height(16), ...List_generate(steps.size(), (index) -> _buildStepItem(steps.get(index), index, expandedSteps.get(index), currentStep == index)))))))))));
}
...
.fontSize(12)
.color(Colors.black87())), SizedBox()
.height(16), ...List_generate(step.subSteps.size(), (subIndex) -> _buildSubStepItem(step.subSteps.get(subIndex), index, subIndex, step.subSteps.get(subIndex).isCompleted, currentStep == index && currentSubSteps.get(index) == subIndex)))))) : null).filter(Objects::nonNull).toList())));
}
